...
//PID variables
double kp = 3E-4, ki = 2E-6, kd = 9E-3;                                                       //PID constants
...
//Other Variables
double ff_scale = 1E-2;          //feed forward scale, how much accleration affects tilt for shape
...
void loop() {
  ...
  ellipsePattern(80, 80, 0, 20, 5);  //moves the ball in an elipse (rx, ry, start, wait, num)
  //sinusoidalPattern(50, 30, 20);  //moves ball in a sinusoidal pattern (A, B, wait)
  // figure8Pattern(130, 0, 30, 5);  //moves the ball in an elipse (r, start, wait, num)
...
void linePattern(double rx, int ry, unsigned long wait, int num) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    for (double j = 0; j <= 2 * PI; j += 0.1) {
      ...
      while (millis() - timeII < wait) {        //moves the ball
        ...
        out[0] += ff_scale * (-rx * cos(theta));
        out[1] += ff_scale * (-ry * sin(theta));
...
void sinusoidalPattern(double ampli, double freq, int wait) {
  ...
  for (double X = -300; X <= 300; X += 5) {
    ...
    while (millis() - timeII < wait) {  //moves the ball
      PID(X, ampli * sin(X / freq));    //(X setpoint, Y setpoint)
      // out[1] += ff_scale * (-ampli * sin(X / freq) / (freq * freq));
      // out[1] = constrain(out[1], -0.25, 0.25);
    }
  }
}
...
void figure8Pattern(double r, double start, int wait, int num) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    for (double j = 0; j < 2 * PI; j += 0.05) {
      ...
      while (millis() - timeII < wait) {
        // PID(scale * cos(theta), scale * sin(2 * theta) / 1.5);
        PID(scale * sin(2 * theta) / 1.5, scale * cos(theta));  //(X setpoint, Y setpoint)
        // out[0] += ff_scale * (-scale * cos(theta));
        // out[1] += ff_scale * (-scale * sin(2 * theta) / 1.5);
        // out[0] = constrain(out[0], -0.25, 0.25);
        // out[1] = constrain(out[1], -0.25, 0.25);
      }
